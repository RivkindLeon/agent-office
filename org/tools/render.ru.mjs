...
const TASKS_RU = {
  ...
  "developer-review-request": (t) => `прочитать работу разработчика по проекту ${t.target} — projects/${t.target}/REVIEW-REQUEST.md`,
  "hiring-manager-needs-substance": (t) => `написать существо работы в заявку на ${t.role} — ${t.requisition}`,
  "subordinate-package-needs-substance": (t) => `вынести вердикт по существу на пакет ${t.role} ${t.version}`,
...
const STEPS_RU = {
  ...
  review: "прочитать изменение по org/PATTERN-CODE-REVIEW.md: одобрять улучшение, а не идеал; блокер отделять от `Nit:`; не давать работе залёживаться",
  substance: "дописать в заявку раздел «Существо работы»: что должность обязана уметь, что считать хорошей работой, из каких шагов состоит её работа",
  "substance-review": "прочитать пакет и вынести вердикт `dimension: substance`: та ли это работа, а не правильно ли она оформлена",
