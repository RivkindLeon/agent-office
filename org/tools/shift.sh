...
# Один замок на всю компанию: две смены одновременно пишут в одно дерево и
# коммитят друг друга. Проверено вживую 2026-09-06 - два цикла запустились
# параллельно и разошлись бы в git.
LOCK="$HOME/.agent-office/shift.lock"
mkdir -p "$(dirname "$LOCK")"
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "смена уже идёт, выхожу"
  exit 0
fi
